...
const Game = {
    ...
    handleSkipSchool: function() {
        const alreadyHasDetention = this.state.school.saturdayDetention;
        const roll = Math.random();

        // If Saturday detention is already pending, another caught skip must become
        // a harsher home punishment instead of stacking duplicate detention.
        if (alreadyHasDetention) {
            if (roll < 0.50) {
                return {
                    result: 'punishment_chores',
                    message: 'You got caught skipping again while detention is already upcoming! Your parents assigned extra punishment chores today.'
                };
            }
            this.groundPlayer(7);
            return {
                result: 'grounded',
                message: 'You got caught skipping again while detention is already upcoming! Your parents grounded you for a week - no allowance and mandatory chores each day.'
            };
        }

        if (roll < 0.35) {
            // 35% chance: Got away with it
            return {
                result: 'success',
                message: 'You successfully skipped school without getting caught!'
            };
        } else if (roll < 0.55) {
            // 20% chance: Saturday detention
            ...
            // 20% chance: Immediate punishment chores
            return {
                result: 'punishment_chores',
                message: 'You got caught! Your parents assigned punishment chores for skipping school.'
            };
        } else {
    ...
    // Ground the player for a number of days
    groundPlayer: function(days) {
        this.state.school.grounded = true;
        this.state.school.groundedDaysLeft = Math.max(this.state.school.groundedDaysLeft, days);
        this.state.school.noAllowanceDaysLeft = Math.max(this.state.school.noAllowanceDaysLeft, days);
    },
    ...
    applyTimeEffects: function() {
        // Core stats slip faster if not maintained, making upkeep more demanding.
        const dailyDecay = {
            happiness: -2,
            intelligence: -1,
            popularity: -1,
        ...
        for (const [stat, amount] of Object.entries(dailyDecay)) {
            if (this.state.stats[stat] > 20) {
